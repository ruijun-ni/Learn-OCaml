...
let rec normalize c =
  match c with 
       If(If (pi, a1, b1), a2, b2) -> normalize (If((normalize pi), (If((normalize a1), a2, b2)), (If((normalize b1), a2, b2)))) |
...
let rec simplify c =
  match c with 
       If(True, a, b) -> simplify a |
       If(False, a, b) -> simplify b |
       If(pi, a, b) ->
          (match pi with 
               Var x ->
                  if (simplify (substitute a x True)) = (simplify (substitute b x False)) then 
                    (simplify (substitute a x True))
                  else If(pi, (simplify (substitute a x True)), (simplify (substitute b x False))) |
               _ -> if (simplify a) = (simplify b) then 
                      (simplify a)
...
let tautology c =
  simplify(normalize(ifify c)) = True;;
