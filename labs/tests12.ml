(*
   ...
   TESTS 12. The Lisp interpreter LISP.ML, modified with tests for Lab 12.
...
(*
   ...
   This is the Lisp interpreter that's been discussed in past lectures.
*)
...
primitive "number"
    (fun args env->
        match args 
        with Nil -> Nil |
             Cons(Number _, Nil) -> tee |
             Cons(arg,Nil) ->
                let a = evaluating arg env 
                in (match a 
                    with Number _ -> tee |
                         _ -> Nil) |
            _ -> oops "EvaluatorError") ;;
...
primitive "let"
    (fun args env -> 
        match args 
        with Cons(Symbol x, Cons(y, Cons(z, Nil))) ->
                global := envPut x (evaluating y env) (!global);
                evaluating z (!global) |
             _ -> oops "EvaluatorError");;
